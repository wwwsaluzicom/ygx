...
class PMF_Category
{
    ...
    public function viewTree()
    {
        ...
        for ($y = 0;$y < $this->height(); $y = $this->getNextLineTree($y)) {
            ...
            if ($level > $open) {
                $output .= sprintf(
                    '<ul><li class="list-group-item" data-category-id="%d" data-category-level="%d">',
                    $parent,
                    $level
                );
            } else {
                $output .= sprintf(
                    '<li class="list-group-item" data-category-id="%d" data-category-level="%d">',
                    $parent,
                    $level
                );
            }
            ...
            if (0 === $number[$parent] && 0 === $level) {
                ...
                $numFaqs = '<span> ('.$plr->GetMsg('plmsgEntries', $number[$parent]);
                $numFaqs .= ')</span>';
            }
    ...
    public function getPath($id, $separator = ' / ', $renderAsMicroData = false, $useCssClass = 'breadcrumb')
    {
        ...
        if ($renderAsMicroData) {
            foreach ($temp as $k => $category) {
                ...
                $breadcrumb[] = sprintf(
                    '<li class="breadcrumb-item" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">%s</li>',
                    $oLink->toHtmlAnchor()
                );
            }
